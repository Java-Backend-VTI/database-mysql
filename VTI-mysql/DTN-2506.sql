...
select * from sinhVien where masv in (select masv from HuongDan where madt is null);
...
CREATE OR REPLACE VIEW vw_SinhVienInfo AS-- tao view
...
select s.masv,s.hoten, COALESCE(d.tendt, 'Chưa có') AS tendt
from sinh_vien s
left join huong_dan h on s.masv=h.masv
left join de_tai d on h.madt=d.madt;
...
-- truoc hay sau (kiem tra dkien thõa mãn trước hay sau khi thực hiện hành động)  before
-- them sua hay xoa voi bang nao -- insert   sinhvien
-- new(chua co) old(da co)
-- them moi(du lieu chua có terong DB) new 
-- delete (đã có rồi thì ms xóa dc) old
-- update   update   tbaleA  set   name= ? , age = ?   where id = 10;
								-- new 		new				  old

DROP TRIGGER trg_bfInsertOnSv;
DELIMITER $$
	CREATE TRIGGER trg_bfInsertOnSv
    BEFORE INSERT ON sinhvien
    FOR EACH ROW
    BEGIN		
        IF NEW.namsinh <'1900-01-01' OR NEW.namsinh is null  THEN
...
INSERT INTO ThucTap1.SinhVien     ( `hoten`,			`quequan`)
VALUES    						 ('SinhVien 6',		 	 'ND');
